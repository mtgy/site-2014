...
var Composition = Edge.Composition, Symbol = Edge.Symbol; // alias pour les classes Edge couramment utilisées
   ...
   //Edge symbol: 'stage'
   (function(symbolName) {
      $("#Stage").css("background-image","url(back.jpg)","background-repeat","repeat")
      ...
      // Masquer un élément.
      // (sym.$("name") résout un nom d’élément Edge Animate en élément DOM
      // pouvant être utilisé avec jQuery)
      //sym.getComposition().getStage().getSymbol("pan_txt").hide();
      //sym.getComposition().getStage().getSymbol("pan_txt").stop();
      ...
      Symbol.bindElementAction(compId, symbolName, "${_py_soudan}", "mouseleave", function(sym, e) {
         sym.getComposition().getStage().getSymbol("py_soudan").playReverse();
         //this.playReverse();
      
      });
      //Edge binding end
      
      Symbol.bindElementAction(compId, symbolName, "${_py_soudan}", "mouseenter", function(sym, e) {
         sym.getComposition().getStage().getSymbol("py_soudan").play(1); 
      
      });
      //Edge binding end
      
      Symbol.bindElementAction(compId, symbolName, "${_py_soudan}", "click", function(sym, e) {
         
         sym.play(751);
         
         sym.getSymbol("pan_txt").$("texte").html("Les deux pays détiennent les cinquièmes réserves de pétrole en Afrique, sauf que le Sud détient près des trois quarts des champs pétrolifères, et le Nord toutes les infrastructures. Sans accord clair, le Sud-Soudan et le Soudan n’ont pas d’autres choix que de s’entendre. Avant la partition du pays, le Soudan était le sixième exportateur de brut du continent avec près de 500 000 barils par jour. La China National Petroleum Company (CNPC) est le plus gros investisseur étranger du pays, avec quelque 5 milliards de dollars dans le développement de champs pétroliers. Elle possède 50 % d’une raffinerie de pétrole, près de Khartoum, en partage avec le gouvernement soudanais.");
         
         sym.getSymbol("pan_txt").$("Text").html("Soudan et Sud-Soudan");
         
         sym.getSymbol("pan_txt").$("fond_txt").css('height','325px');

      });
      //Edge binding end
      
      Symbol.bindElementAction(compId, symbolName, "${_py_tchad}", "mouseenter", function(sym, e) {
         sym.getComposition().getStage().getSymbol("py_tchad").play(1); 
      
      });
      //Edge binding end
      
      Symbol.bindElementAction(compId, symbolName, "${_py_tchad}", "mouseleave", function(sym, e) {
         sym.getComposition().getStage().getSymbol("py_tchad").playReverse();
      ...
      Symbol.bindElementAction(compId, symbolName, "${_py_tchad}", "click", function(sym, e) {
         
         sym.play(751);
         
         
         //sym.getComposition().getStage().getSymbol("pan_txt").getSymbol("Text").html("Tchad");
         
         
         
         //sym.getComposition().getStage().sym.getSymbol("pan_txt").sym.$("texte").html("Depuis dix ans, le pays est devenu un pays exportateur de pétrole, avec des réserves certifiées de 1,5 milliard de barils concentrés autour de trois principaux gisements de Doba-Komé, Miandoum et Bolobo. L’exploitation revient principalement aux Américains via le consortium ExxonMobil, Chevron (États-Unis) et Petronas (Malaisie). La Chine, qui depuis plusieurs années cherche à sécuriser son approvisionnement en matière de ressources naturelles, est également bien implantée dans les secteurs pétrolier et gazier malgré une mésentente avec les autorités tchadiennes sur les prix.");
         
         //sym.getSymbol("pan_txt").$("texte").hide();
         
         sym.getSymbol("pan_txt").$("texte").html("Depuis dix ans, le pays est devenu un pays exportateur de pétrole, avec des réserves certifiées de 1,5 milliard de barils concentrés autour de trois principaux gisements de Doba-Komé, Miandoum et Bolobo. L’exploitation revient principalement aux Américains via le consortium ExxonMobil, Chevron (États-Unis) et Petronas (Malaisie). La Chine, qui depuis plusieurs années cherche à sécuriser son approvisionnement en matière de ressources naturelles, est également bien implantée dans les secteurs pétrolier et gazier malgré une mésentente avec les autorités tchadiennes sur les prix.");
         
         sym.getSymbol("pan_txt").$("Text").html("Tchad");
         sym.getSymbol("pan_txt").$("fond_txt").css('height','288px');

      });
      //Edge binding end

      Symbol.bindElementAction(compId, symbolName, "${_close_hide}", "click", function(sym, e) {
         sym.playReverse();

      });
      //Edge binding end

      Symbol.bindTriggerAction(compId, symbolName, "Default Timeline", 499, function(sym, e) {
         sym.stop();

      });
      //Edge binding end

      Symbol.bindElementAction(compId, symbolName, "${_btn_intro}", "click", function(sym, e) {
         // lire le scénario à partir de la position spécifiée (ms ou étiquette)
         sym.play(500);
         
         //$("intro").sym.play(500);
         //sym.getSymbol("intro").play(501);
      ...
      Symbol.bindElementAction(compId, symbolName, "${_py_niger}", "mouseenter", function(sym, e) {
         sym.getComposition().getStage().getSymbol("py_niger").play(1); 

      });
      //Edge binding end

      Symbol.bindElementAction(compId, symbolName, "${_py_niger}", "mouseleave", function(sym, e) {
         sym.getComposition().getStage().getSymbol("py_niger").playReverse();

      });
      //Edge binding end

      Symbol.bindElementAction(compId, symbolName, "${_py_niger}", "click", function(sym, e) {
         sym.play(751);
         
         sym.getSymbol("pan_txt").$("texte").html("L’uranium place le Niger au deuxième rang des producteurs mondiaux. Et la France, qui a bénéficié par le passé d’un accès exclusif à l’exploitation de ce minerai, est le premier investisseur étranger dans ce domaine. La société française Areva y importe actuellement près de 40 % de son uranium et s’est assuré les droits de la mine géante d’Imouraren, potentiellement la deuxième plus grande du monde. Toutefois, la multinationale française est aujourd’hui confrontée à l’arrivée des sociétés concurrentes américaines, chinoises, ou encore sud-coréennes.<br/><br/>Depuis 2012, le Niger a rejoint la liste des pays producteurs et exportateurs d’or noir grâce à ses champs pétrolifères situés dans la zone d'Agadem, dans l’extrême-est du pays. À l’heure actuelle, ses réserves sont estimées à plus de 300 millions de barils. Dans ce secteur, la Chine est devenue un partenaire commercial important et un investisseur majeur.");
         
         sym.getSymbol("pan_txt").$("Text").html("Niger");
         
         sym.getSymbol("pan_txt").$("fond_txt").css('height','447px');

      });
      //Edge binding end

      Symbol.bindElementAction(compId, symbolName, "${_py_mali}", "mouseenter", function(sym, e) {
         sym.getComposition().getStage().getSymbol("py_mali").play(1);

      });
      //Edge binding end

      Symbol.bindElementAction(compId, symbolName, "${_py_mali}", "mouseleave", function(sym, e) {
         sym.getComposition().getStage().getSymbol("py_mali").playReverse();

      });
      //Edge binding end

      Symbol.bindElementAction(compId, symbolName, "${_py_mali}", "click", function(sym, e) {
         sym.play(751);
         ...
         sym.getSymbol("pan_txt").$("Text").html("Mali");
   ...
   (function(symbolName) {   
      ...
      Symbol.bindElementAction(compId, symbolName, "${_close_btn}", "click", function(sym, e) {
         //sym.playReverse();// insérer le code du clic de souris ici
         //$("#Stage").playReverse();
...
sym.stop();
      ...
      Symbol.bindElementAction(compId, symbolName, "${_txt_close_btn}", "mouseenter", function(sym, e) {
         sym.play(1);

      });
      //Edge binding end

      Symbol.bindElementAction(compId, symbolName, "${_txt_close_btn}", "mouseleave", function(sym, e) {
         sym.playReverse();
      ...
      Symbol.bindElementAction(compId, symbolName, "${_txt_close_btn}", "click", function(sym, e) {
