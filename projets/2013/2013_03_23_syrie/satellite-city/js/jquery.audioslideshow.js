	...
	$.fn.audioSlideshow = function( options ) {
		...
		return this.each(function () {
			...
			$jPlayerObj.bind($.jPlayer.event.timeupdate, function(event) { // Add a listener to report the time play began
				...
				if (p == '100%') {
					window.location = '../la-camera-pour-arme/';
				}
				...
				if(slidesCount){
					...
					var intro = $(".audio-slides img.active").attr('data-intro')
					if (intro)
						$('#data-intro').html(intro);
					else 
						$('#data-intro').html('');
						
					$('#LaDivDeCaption').html($(".audio-slides img.active").attr('alt'));
				}
			});
			...
			function setAudioSlide(n){
				if(n != currentSlide){
					if($slides.get(currentSlide)){
						$($slides.get(currentSlide)).fadeOut(300,'swing',function(){
							$($slides.get(currentSlide)).removeClass('active');
						});
						setTimeout(function(){
							$($slides.get(n)).fadeIn(250,'swing',function(){
								$($slides.get(n)).addClass('active');currentSlide = n;
							});
						},250);						
					} else {
						$($slides.get(n)).fadeIn(250,'swing',function(){
